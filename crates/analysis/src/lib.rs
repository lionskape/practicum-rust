...
/// Ошибка выбора режима чтения.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ReadLogError {
    /// Неизвестный числовой идентификатор режима.
    UnknownMode(u8),
}
...
impl std::fmt::Display for ReadLogError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnknownMode(mode) => write!(f, "unknown read mode {mode}"),
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ReadMode {
    /// Читать все распарсенные строки.
    All,
    /// Читать только ошибки.
    Errors,
    /// Читать только операции, меняющие состояние счетов и активов.
    Exchanges,
}

impl TryFrom<u8> for ReadMode {
    type Error = ReadLogError;
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            1 => Ok(Self::Errors),
            2 => Ok(Self::Exchanges),
            other => Err(ReadLogError::UnknownMode(other)),
        }
    }
}
...
impl<R: Read> LogIterator<R> {
    fn new(input: R) -> Self {
        Self { lines: std::io::BufReader::with_capacity(4096, input).lines() }
    }
}
...
impl<R: Read> Iterator for LogIterator<R> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let (remaining, result) = parse::parse_log_line(line).ok()?;
            return remaining.trim().is_empty().then_some(result);
        }
    }
}

/// Принимает поток байт, отдаёт отфильтрованные и распарсенные логи
pub fn read_log<R: Read>(input: R, mode: ReadMode, request_ids: &[u32]) -> Vec<LogLine> {
    LogIterator::new(input)
        .filter(|log| {
            (request_ids.is_empty() || request_ids.contains(&log.request_id)) && mode.accepts(log)
        })
        .collect()
}

/// Принимает старый числовой режим, валидирует его и читает логи без паники.
pub fn read_log_by_mode_id<R: Read>(
    input: R,
    mode: u8,
    request_ids: &[u32],
) -> Result<Vec<LogLine>, ReadLogError> {
    Ok(read_log(input, ReadMode::try_from(mode)?, request_ids))
}
...
App::Journal BuyAsset UserBacket{"user_id":"Alice","backet":Backet{"asset_id":"milk","count":5,},} requestid=10
    ...
    #[test]
    fn invalid_read_mode_is_reported() {
        assert_eq!(
            read_log_by_mode_id(SOURCE1.as_bytes(), 99, &[]),
            Err(ReadLogError::UnknownMode(99))
        );
    }
}
