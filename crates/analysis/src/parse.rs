...
trait Parser {
    type Dest;
    fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)>;
}
...
mod stdp {
    ...
    impl Parser for U32 {
        type Dest = u32;
        fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
            let (remaining, is_hex) =
                input.strip_prefix("0x").map_or((input, false), |remaining| (remaining, true));
            ...
            let value = std::num::NonZeroU32::new(value).ok_or(ParseError)?.get();
    ...
    impl Parser for I32 {
        ...
        fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
            let end_idx = input
                ...
                .unwrap_or(input.len());
            let value = input[..end_idx].parse().map_err(|_| ParseError)?;
            let value = std::num::NonZeroI32::new(value).ok_or(ParseError)?.get();
    ...
    impl Parser for Byte {
        type Dest = u8;
        fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
            let (to_parse, remaining) = input.split_at_checked(2).ok_or(ParseError)?;
            if !to_parse.chars().all(|c| c.is_ascii_hexdigit()) {
                return Err(ParseError);
            }
            let value = u8::from_str_radix(to_parse, 16).map_err(|_| ParseError)?;
            Ok((remaining, value))
        }
    }
}
...
fn do_unquote(input: &str) -> ParseResult<(&str, String)> {
    let mut result = String::new();
    let mut escaped_now = false;
    let mut chars = input.strip_prefix("\"").ok_or(ParseError)?.chars();
    ...
    Err(ParseError) // строка кончилась, не закрыв кавычку
}
/// Распарсить строку, обёрную в кавычки
/// (сокращённая версия [do_unquote], в которой вложенные кавычки не предусмотрены)
fn do_unquote_non_escaped(input: &str) -> ParseResult<(&str, String)> {
    let input = input.strip_prefix("\"").ok_or(ParseError)?;
    let quote_byteidx = input.find('"').ok_or(ParseError)?;
...
impl Parser for Tag {
    type Dest = ();
    fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
        Ok((input.strip_prefix(self.tag).ok_or(ParseError)?, ()))
    }
}
/// Конструктор [Tag]
...
/// Конструктор [All] для любого поддержанного набора парсеров.
fn all<T>(parser: T) -> All<T> {
    All { parser }
}
...
type KeyValueParser<T> = Delimited<
...
fn key_value<T: Parser>(key: &'static str, value_parser: T) -> KeyValue<T> {
    KeyValue {
        parser: delimited(
            all((strip_whitespace(quoted_tag(key)), strip_whitespace(tag(":")))),
    ...
    A1: Parser,
{
    ...
    fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
        match self.parsers.0.parse(input) {
            Ok((remaining, a0)) => {
                self.parsers.1.parse(remaining).map(|(remaining, a1)| (remaining, (a0, a1)))
            }
            Err(ParseError) => self.parsers.1.parse(input).and_then(|(remaining, a1)| {
    ...
    fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
        match self.parsers.0.parse(input) {
            Ok((remaining, a0)) => match self.parsers.1.parse(remaining) {
                Ok((remaining, a1)) => {
                    self.parsers.2.parse(remaining).map(|(remaining, a2)| (remaining, (a0, a1, a2)))
                }
                Err(ParseError) => self.parsers.2.parse(remaining).and_then(|(remaining, a2)| {
                    self.parsers.1.parse(remaining).map(|(remaining, a1)| (remaining, (a0, a1, a2)))
                }),
            },
            Err(ParseError) => {
                match self.parsers.1.parse(input) {
                    ...
                    Err(ParseError) => self.parsers.2.parse(input).and_then(|(remaining, a2)| {
...
impl<T: Parser> Parser for List<T> {
    ...
    fn parse<'a>(&self, input: &'a str) -> ParseResult<(&'a str, Self::Dest)> {
        let mut remaining = input.trim_start().strip_prefix('[').ok_or(ParseError)?.trim_start();
        ...
        Err(ParseError) // строка кончилась, не закрыв скобку
    }
}
...
/// Конструктор [Alt] для любого поддержанного набора парсеров.
fn alt<T>(parser: T) -> Alt<T> {
    Alt { parser }
}
...
impl Parsable for AssetDsc {
    ...
    fn parser() -> Self::Parser {
        ...
        map(
            delimited(
                all((strip_whitespace(tag("AssetDsc")), strip_whitespace(tag("{")))),
...
impl Parsable for Backet {
    ...
    fn parser() -> Self::Parser {
        map(
            delimited(
                all((strip_whitespace(tag("Backet")), strip_whitespace(tag("{")))),
...
impl Parsable for UserCash {
    ...
    fn parser() -> Self::Parser {
        map(
            delimited(
                all((strip_whitespace(tag("UserCash")), strip_whitespace(tag("{")))),
...
impl Parsable for UserBacket {
    ...
    fn parser() -> Self::Parser {
        map(
            delimited(
                all((strip_whitespace(tag("UserBacket")), strip_whitespace(tag("{")))),
                permutation2(
...
impl Parsable for UserBackets {
    ...
    fn parser() -> Self::Parser {
        map(
            delimited(
                all((strip_whitespace(tag("UserBackets")), strip_whitespace(tag("{")))),
                permutation2(
                    key_value("user_id", unquote()),
...
fn parse_owned<T: Parsable>(input: String) -> ParseResult<(String, T)> {
    let (remaining, parsed) = T::parser().parse(&input)?;
    Ok((remaining.to_string(), parsed))
}

// просто обёртки
/// Обёртка для парсинга [AssetDsc]
pub fn just_parse_asset_dsc(input: String) -> ParseResult<(String, AssetDsc)> {
    parse_owned::<AssetDsc>(input)
}
/// Обёртка для парсинга [Backet]
pub fn just_parse_backet(input: String) -> ParseResult<(String, Backet)> {
    parse_owned::<Backet>(input)
}
/// Обёртка для парсинга [UserCash]
pub fn just_user_cash(input: String) -> ParseResult<(String, UserCash)> {
    parse_owned::<UserCash>(input)
}
/// Обёртка для парсинга [UserBacket]
pub fn just_user_backet(input: String) -> ParseResult<(String, UserBacket)> {
    parse_owned::<UserBacket>(input)
}
/// Обёртка для парсинга [UserBackets]
pub fn just_user_backets(input: String) -> ParseResult<(String, UserBackets)> {
    parse_owned::<UserBackets>(input)
}
/// Обёртка для парсинга [Announcements]
pub fn just_parse_anouncements(input: String) -> ParseResult<(String, Announcements)> {
    parse_owned::<Announcements>(input)
}
...
fn connect_log(authdata: AuthData) -> AppLogTraceKind {
...
impl Parsable for LogLine {
    ...
    fn parser() -> Self::Parser {
        map(
            all((LogKind::parser(), strip_whitespace(preceded(tag("requestid="), stdp::U32)))),
            |(kind, request_id)| LogLine { kind, request_id },
        )
    }
}

/// Распарсить строку логов.
pub fn parse_log_line(input: &str) -> ParseResult<(&str, LogLine)> {
...
mod test {
    ...
    fn test_u32() {
        ...
        assert_eq!(stdp::U32.parse(""), Err(ParseError));
        assert_eq!(stdp::U32.parse("-3"), Err(ParseError));
        assert_eq!(stdp::U32.parse("0x03"), Ok(("", 0x3)));
        assert_eq!(stdp::U32.parse("0x03abg"), Ok(("g", 0x3ab)));
        assert_eq!(stdp::U32.parse("0x"), Err(ParseError));
    }
    ...
    fn test_i32() {
        ...
        assert_eq!(stdp::I32.parse(""), Err(ParseError));
        assert_eq!(stdp::I32.parse("-3"), Ok(("", -3)));
        assert_eq!(stdp::I32.parse("0x03"), Err(ParseError));
        assert_eq!(stdp::I32.parse("-"), Err(ParseError));
    }
    ...
    fn test_do_unquote_non_escaped() {
        ...
        assert_eq!(do_unquote_non_escaped(r#" "411""#), Err(ParseError));
        assert_eq!(do_unquote_non_escaped(r#"411"#), Err(ParseError));
    }
    ...
    fn test_unquote() {
        ...
        assert_eq!(Unquote.parse(r#" "411""#), Err(ParseError));
        assert_eq!(Unquote.parse(r#"411"#), Err(ParseError));
    ...
    fn test_tag() {
        ...
        assert_eq!(tag("key=").parse("key:value"), Err(ParseError));
    }

    #[test]
    fn test_quoted_tag() {
        assert_eq!(quoted_tag("key").parse(r#""key"=value"#), Ok(("=value", ())));
        assert_eq!(quoted_tag("key").parse(r#""key:"value"#), Err(ParseError));
        assert_eq!(quoted_tag("key").parse(r#"key=value"#), Err(ParseError));
    }
    ...
    fn test_delimited() {
        ...
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("0x32]"), Err(ParseError));
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32"), Err(ParseError));
    }
    ...
    fn test_key_value() {
        ...
        assert_eq!(key_value("key", stdp::U32).parse(r#"key:32,"#), Err(ParseError));
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32"#), Err(ParseError));
    ...
    fn test_list() {
        ...
        assert_eq!(list(stdp::U32).parse("1,2,3,4,"), Err(ParseError));
    ...
    fn test_asset_dsc() {
        ...
        assert_eq!(
            ...
            Ok(("", AssetDsc { id: "usd".into(), dsc: "USA dollar".into() }))
        );
        assert_eq!(
            AssetDsc::parser().parse(r#" AssetDsc { "id" : "usd" , "dsc" : "USA dollar" , } "#),
            Ok(("", AssetDsc { id: "usd".into(), dsc: "USA dollar".into() }))
        ...
        assert_eq!(
            ...
            Ok(("nice ", AssetDsc { id: "usd".into(), dsc: "USA dollar".into() }))
        );
    ...
    fn test_backet() {
        assert_eq!(
            ...
            Ok(("", Backet { asset_id: "usd".into(), count: 42 }))
        );
        assert_eq!(
    ...
    fn test_log_kind() {
        ...
        assert_eq!(
            ...
            Ok((
                "",
                LogKind::App(AppLogKind::Journal(AppLogJournalKind::DepositCash(UserCash {
                    user_id: "Steeve".into(),
                    count: 10
                })))
            ))
        ...
        assert_eq!(
            ...
            Ok((
                "",
                LogKind::App(AppLogKind::Journal(AppLogJournalKind::WithdrawCash(UserCash {
                    user_id: "Steeve".into(),
                    count: 10
                })))
            ))
